...
class QuranProvider extends ChangeNotifier {
  /// Class Constructor
  QuranProvider() {
    print('flutter: [STARTUP] 3. QuranProvider init');
    getAllSurahs();
    getLocalSettingOfQuran();
    print('flutter: [STARTUP] 4. QuranProvider TranslationService');
  ...
  void changeQuranType(int index) {
    ...
    // Defer write and notify to next frame so navigation doesn't jam (avoids rebuilding previous route during push).
  ...
  void changeReadingType(EReadOptions readOptions) {
    localSetting.readOptions = readOptions;
    setLocalSettingOfQuran();
  }

  /// Changing Layout Options
  void changeLayoutOptions(ELayoutOptions layoutOptions) {
    localSetting.layoutOptions = layoutOptions;
    setLocalSettingOfQuran();
  }

  /// Changing Font Size
  void changeFontSize(dynamic value) {
    localSetting.textScaleFactor = value;
    setLocalSettingOfQuran();
  }

  /// Changing vertical space between lines (line height multiplier).
  void changeLineHeight(double value) {
    localSetting.lineHeight = value.clamp(1.0, 2.5);
    setLocalSettingOfQuran();
    notifyListeners();
  }
  ...
  void changeFontTypeArabic(String value) {
    localSetting.fontTypeArabic = value;
    setLocalSettingOfQuran();
  }

  /// Verse text as display spans (same font/styling for Reading and Translation).
  /// Override or extend to add Tajweed colors when data is available.
  List<InlineSpan> getVerseDisplaySpans(VerseModel verse, TextStyle baseStyle) {
  ...
  void changeSurahDetailsPageTheme(int index) {
