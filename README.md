...
### Clean and build (if you need a fresh build)

If you have to delete the `build` folder every time before debugging or building the APK, use Flutter’s clean command instead:

* **Clean only:**  
  `flutter clean`  
  Then run `flutter run` or `flutter build apk` as usual.

* **Convenience scripts (Windows):**
  * **`clean_run.bat`** – runs `flutter clean` then `flutter run` (for debugging).
  * **`clean_build.bat`** – runs `flutter clean` then `flutter build apk` (output: `build\app\outputs\flutter-apk\app-release.apk`).

You don’t need to manually delete the `build` folder; `flutter clean` clears build cache and is the recommended way to get a fresh build.
